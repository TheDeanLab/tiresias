...
def psf_to_otf(psf: Array, output_shape: Sequence[int], *, xp: Any = np) -> Array:
    ...
    if any(
        int(psf_size) > output_size
        for psf_size, output_size in zip(kernel.shape, target_shape)
    ):
        raise ValueError(
...
def _release_cupy_workspace(cp: Any) -> None:
    """Release shape-specific FFT plans and pooled blocks between chunks."""
...
def accelerated_richardson_lucy(
    observed: Array,
    psf: Array,
    n_iters: int,
    *,
    ...
    """Apply accelerated simplified Richardson-Lucy iterations."""
    n_iters = int(n_iters)
    ...
    if any(
        int(psf_size) > int(image_size)
        for psf_size, image_size in zip(kernel.shape, image.shape)
    ):
        raise ValueError(
    ...
    if not np.isfinite(kernel_sum) or kernel_sum <= 0:
        raise ValueError(f"PSF must have a positive finite sum, got {kernel_sum}")
    ...
    for iteration in range(n_iters):
        ...
        extrapolated = xp.maximum(current + acceleration * (current - previous), 0)
        ...
        previous = current
        correction = xp.fft.ifftn(transfer_adjoint * xp.fft.fftn(ratio)).real
...
def deconvolve_with_cupy(
    ...
    *,
    ...
    """Restore an image with CuPy FFT Richardson-Lucy using the estimated PSF."""
    try:
        import cupy as cp
    except ImportError as exc:
    ...
    with cp.cuda.Device(int(device_id)):
        try:
            ...
            restored_gpu = accelerated_richardson_lucy(
                image_gpu,
                psf_gpu,
                n_iters,
